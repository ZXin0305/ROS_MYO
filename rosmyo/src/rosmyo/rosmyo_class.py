...
class RosMyo:
	...
	def __init__(self):
		# ROS init
		rospy.init_node(RosMyo.NODE_DEFAULT_NAME, anonymous=True)
		...
		self.__myo = Myo(sp)  # serial_port to connect the myo device
	...
	def pub_emg(self, value):
		e = Emg()   # built the .msg classers to store emg values..
	...
	def run(self):
		...
		if not self.__myo.connect(self.__myo_identifier):   #try to connect device
